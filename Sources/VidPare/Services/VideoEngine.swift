...
final class VideoEngine {
    ...
    for quality in QualityPreset.allCases {
      ...
      guard let session = AVAssetExportSession(asset: asset, presetName: preset) else {
        for format in ExportFormat.allCases {
          row[format] = .unsupported(
            "The \(quality.rawValue) preset is not compatible with this source.")
        }
        matrix[quality] = row
        continue
      }
      ...
      for format in ExportFormat.allCases {
        if quality.isPassthrough {
          if format != sourceContainer {
            row[format] = .unsupported(
              "Passthrough keeps the source container (\(sourceContainer.containerLabel)).")
          } else if supportedFileTypes.contains(sourceContainer.fileType) {
            row[format] = .supported
          } else {
            row[format] = .unsupported(
              "Passthrough is unavailable for this source container on this Mac.")
          }
          continue
        }
        ...
        guard supportedFileTypes.contains(format.fileType) else {
          row[format] = .unsupported(
            "The \(format.containerLabel) container is not available with \(quality.rawValue) on this Mac."
          )
          continue
        }
  ...
  private func resolveRequestedSelection(
    ...
    let requestedFormat =
      requestedQuality.isPassthrough ? capabilities.sourceContainerFormat : format

    guard capabilities.isSupported(format: requestedFormat, quality: requestedQuality) else {
      let reason =
        capabilities.support(for: requestedFormat, quality: requestedQuality).reason
  ...
  private func createExportSession(
    ...
    guard
      let session = AVAssetExportSession(asset: asset, presetName: requestedQuality.exportPreset)
    else {
...
enum ExportError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .sessionCreationFailed:
      return
        "Failed to create export session. The file format may not support the selected export settings."
