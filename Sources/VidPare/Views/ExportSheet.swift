...
struct ExportSheet: View {
  ...
  private var selectionWarning: String? {
    ...
    let support = capabilities.support(
      for: trimState.exportFormat, quality: trimState.qualityPreset)
  ...
  var body: some View {
    Group {
      if let result = exportResult {
        ExportCompletionView(result: result, onDismiss: onDismiss)
      } else {
        exportOptionsView
      }
    }
  ...
  private var exportOptionsView: some View {
    VStack(spacing: 20) {
      ...
      GroupBox("Trim") {
        HStack {
          Label(
            TimeFormatter.preciseString(from: trimState.startTime),
            systemImage: "arrow.right.to.line"
          )
          Spacer()
          Label(
            TimeFormatter.preciseString(from: trimState.endTime),
      ...
      GroupBox("Format") {
        VStack(alignment: .leading, spacing: 8) {
          ...
          if trimState.qualityPreset.isPassthrough {
            Text(
              "Passthrough keeps the source container (\(document.passthroughContainerFormat.containerLabel))."
            )
            .font(.caption)
  ...
  private func loadCapabilities() async {
    ...
    do {
      ...
      if !resolvedCapabilities.hasAnySupportedOption {
        capabilityMessage =
          "No compatible export options are available for this source on this Mac."
      }
    } catch {
  ...
  private func enforceValidSelection() {
    ...
    let changed =
      resolved.format != trimState.exportFormat || resolved.quality != trimState.qualityPreset
  ...
  private func startExport() {
    ...
    guard capabilities.isSupported(format: resolved.format, quality: resolved.quality) else {
      errorMessage =
        capabilities.support(for: resolved.format, quality: resolved.quality).reason
  ...
  var body: some View {
    VStack(spacing: 16) {
      ...
      VStack(spacing: 4) {
        ...
        Text(
          "Size: \(ByteCountFormatter.string(fromByteCount: result.fileSize, countStyle: .file)) \u{2022} \(String(format: "%.1fs", result.duration))"
        )
        .font(.callout)
        .foregroundStyle(.secondary)
      }
